...
import { IAttachment } from '~/types/vk';
...
export const parseAttachments = (attachments: IAttachment[] = []) =>
  attachments &&
  attachments
    .filter(
      (att) =>
...
export const getAttachmentLinks = (attachments: IAttachment[] = []) =>
  attachments
    .filter(
      (att) =>
        att.type === 'link' && att.link && att.link.button && att.link.button.title === 'Читать'
    )
    .map((att) => ({
...
export const makePostUrl = (group_id: number, post_id: number): string =>
  GROUPS[group_id]
...
export const getMapUrl = (text: string): string => {
  const match =
    text && text.match(/(http|https):\/\/(map|alpha-map)\.vault48\.org\/[0-9A-Za-z\_]+/);
...
export const cutText = (text: string, limit: number): string =>
  `${text.substr(0, limit)}${text.length > limit ? '...' : ''}`;
export const parseText = (text: string) => text.replace(/</gi, '&gt;').replace(/>/gi, '&;lt;');
